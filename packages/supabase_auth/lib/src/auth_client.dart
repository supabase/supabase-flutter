...
/// API client to interact with the Supabase Auth server.
///
/// [url] URL of the Supabase Auth instance
///
...
class AuthClient {
  ...
  /// user's password.
  late final AuthAdminApi admin;

  /// Namespace for the Supabase Auth MFA API methods.
  late final AuthMFAApi mfa;

  /// Namespace for the Supabase Auth OAuth methods.
  late final AuthOAuthApi oauth;

  /// Namespace for the passkey (WebAuthn) API methods.
  ///
  /// {@macro auth_passkey_api}
  @experimental
  late final AuthPasskeyApi passkey;
  ...
  Future<JWK?> _fetchJwk(String kid, JWKSet suppliedJwks) async {
    ...
    final jwksResponse = await _fetch.request(
      ...
      HttpMethod.get,
      options: AuthRequestOptions(headers: _headers),
    );
