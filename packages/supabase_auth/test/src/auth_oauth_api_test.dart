...
void main() {
  late AuthOauthApiFixture fixture;

  setUp(() async {
    fixture = AuthOauthApiFixture();
  });
...
class AuthOauthApiFixture {
  AuthOauthApiFixture() {
    final env = DotEnv();
    env.load(); // Load env variables from .env file

    _authUrl = getAuthUrl(env);
  ...
  Future<AuthClient> build({bool reset = true}) async {
    if (reset) {
      await _reset();
    }
